...
public class XEditText extends AppCompatEditText {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (inputType == 129 || inputType == 145 || inputType == 18 || inputType == 225) {
            ...
            if (mShowPwdResId == -1)
                mShowPwdResId = R.drawable.down_pwd;
            if (mHidePwdResId == -1)
                mHidePwdResId = R.drawable.nor_pwd;

            int tId = isPwdShow ? mShowPwdResId : mHidePwdResId;
            mTogglePwdDrawable = ContextCompat.getDrawable(context, tId);
            if (mShowPwdResId == R.drawable.down_pwd ||
                    mHidePwdResId == R.drawable.nor_pwd) {
            ...
            if (!disableClear) {
                mBitmap = getBitmapFromVectorDrawable(context, cdId,
                        cdId == R.drawable.clear_btn); // clearDrawable
            }
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (hasFocused && mBitmap != null && isPwdInputType && !isTextEmpty()) {
            int left = getMeasuredWidth() - getPaddingRight() - getCompoundDrawablePadding() -
                    mTogglePwdDrawable.getIntrinsicWidth() - mBitmap.getWidth() - dp2px(10);
    ...
    public void setIsDisableClear(boolean disableClear) {
